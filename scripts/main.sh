...
echo -e "${BLUE}============================================================${NC}"
echo -e " ${GREEN}1)${NC} 🏠 ${YELLOW}单机开发 → 升级单区域生产块${NC}"
echo -e "       ${GRAY}└─ 业务数据扩展 ｜ 2台 → 3-6台${NC}"
echo ""
echo -e " ${GREEN}2)${NC} 🌏 ${YELLOW}单区域基础块 → 升级全球化架构${NC}"
echo -e "       ${GRAY}└─ 跨区高可用 ｜ 3-6台 → 7+台${NC}"
echo ""
echo -e " ${GREEN}3)${NC} 🌐 ${YELLOW}企业级全球化 → 多云多活扩展${NC}"
echo -e "       ${GRAY}└─ 全球节点新增 ｜ 7+台 → 无限扩展${NC}"
echo ""
echo -e " ${RED}9)${NC} ⬅️  返回主菜单"
echo -e "${BLUE}============================================================${NC}"
read -p "$(printf "${CYAN}请选择扩容方式：${NC} ")" S_OPT
case "$S_OPT" in
1) info "单机平滑扩容为3-6节点单区域..."; pause ;;
2) info "单区域升级为全球化7+节点架构..."; pause ;;
3) info "企业级全球化架构扩展节点..."; pause ;;
9) return ;;
*) err "无效选项"; sleep 1 ;;
esac
done
