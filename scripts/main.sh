...
case "$MAIN_OPT" in
 ...
 # 节点状态表格
    ...
    printf " 节点1  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-1 k3s)" \
        "$(query_app_state node-1 netbird)" \
        "$(query_app_state node-1 nginx)" \
        "$(query_app_state node-1 newapi)" \
        "$(query_app_state node-1 mysql)" \
        "$(query_app_state node-1 redis)"
    printf " 节点2  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-2 k3s)" \
        "$(query_app_state node-2 netbird)" \
        "$(query_app_state node-2 nginx)" \
        "$(query_app_state node-2 newapi)" \
        "$(query_app_state node-2 mysql)" \
        "$(query_app_state node-2 redis)"
    printf " 节点3  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-3 k3s)" \
        "$(query_app_state node-3 netbird)" \
        "$(query_app_state node-3 nginx)" \
        "$(query_app_state node-3 newapi)" \
        "$(query_app_state node-3 mysql)" \
        "$(query_app_state node-3 redis)"
    printf " 节点4  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-4 k3s)" \
        "$(query_app_state node-4 netbird)" \
        "$(query_app_state node-4 nginx)" \
        "$(query_app_state node-4 newapi)" \
        "$(query_app_state node-4 mysql)" \
        "$(query_app_state node-4 redis)"
    printf " 节点5  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-5 k3s)" \
        "$(query_app_state node-5 netbird)" \
        "$(query_app_state node-5 nginx)" \
        "$(query_app_state node-5 newapi)" \
        "$(query_app_state node-5 mysql)" \
        "$(query_app_state node-5 redis)"
    printf " 节点6  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-6 k3s)" \
        "$(query_app_state node-6 netbird)" \
        "$(query_app_state node-6 nginx)" \
        "$(query_app_state node-6 newapi)" \
        "$(query_app_state node-6 mysql)" \
        "$(query_app_state node-6 redis)"
    printf " 节点7  %-8s %-8s %-8s %-10s %-8s %-8s\n" \
        "$(query_app_state node-7 k3s)" \
        "$(query_app_state node-7 netbird)" \
        "$(query_app_state node-7 nginx)" \
        "$(query_app_state node-7 newapi)" \
        "$(query_app_state node-7 mysql)" \
        "$(query_app_state node-7 redis)"
    echo -e "${BLUE}=====================================================================${NC}"
...
show_plan_minimal() {
    while true; do
        ...
        echo -e "${BLUE}============================================================${NC}"
        echo -e "${GRAY}架构说明：${NC}"
        echo -e "${GRAY}  节点 1 = 控制面 + 业务网关 + 边缘入口${NC}"
        echo -e "${GRAY}  节点 2 = 数据库 + 缓存 + 备份（纯数据，禁业务）${NC}"
        echo -e "${BLUE}============================================================${NC}"
...
show_node_1_menu() {
    while true; do
        ...
        k3s_s=$(query_app_state node-1 k3s)
        netbird_s=$(query_app_state node-1 netbird)
        nginx_s=$(query_app_state node-1 nginx)
        newapi_s=$(query_app_state node-1 newapi)
        mon_s=$(query_app_state node-1 monitoring)

        echo ""
        echo -e "${BLUE}============================================================${NC}"
        echo -e "🖥️  ${WHITE}节点 1（总控机：当前服务器）${NC}"
        echo -e "${BLUE}============================================================${NC}"
        ...
        echo -e "${BLUE}============================================================${NC}"
        echo ""
        echo -e "${WHITE}✅ 可安装应用清单（仅节点 1 允许）：${NC}"
        echo ""
...
show_node_2_menu() {
    while true; do
        ...
        mysql_s=$(query_app_state node-2 mysql)
        redis_s=$(query_app_state node-2 redis)
        netbird_s=$(query_app_state node-2 netbird)

        echo ""
        echo -e "${BLUE}============================================================${NC}"
        echo -e "💾 ${WHITE}节点 2（数据节点：远程服务器）${NC}"
        echo -e "${BLUE}============================================================${NC}"
        echo -e "${CYAN}【主机名】${NC}：${HOSTNAME_INFO}"
        echo -e "${CYAN}【内网 IP】${NC}：${LOCAL_IP}"
        echo -e "${CYAN}【NetBird IP】${NC}：${NETBIRD_IP}"
        ...
        case "$APP_OPT" in
            ...
            5) install_app "proxysql"        "节点 2" ;;
            6) install_app "etcd-backup"     "节点 2" ;;
...
install_app() {
    ...
    if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
        ...
        # 写入全局大脑状态文件
        local node_name
        if [[ "$node" == "节点 1" ]]; then node_name="node-1"; fi
        if [[ "$node" == "节点 2" ]]; then node_name="node-2"; fi
...
show_expand_biz_menu() {
    while true; do
        ...
        echo -e "${CYAN}【时间】${NC}  ：${NOW_TIME}"
        echo -e "${BLUE}------------------------------------------------------------${NC}"
        echo -e "${CYAN}【自动节点名】${NC}：node-3 / node-4 / node-5 依次分配"
...
echo -e "----------------------------------------------------------------${NC}"
echo -e " node-1        国内      管控/控制面    $(get_node_status node-1)"
echo -e " node-2        国内      业务网关       $(get_node_status node-2)"
echo -e " node-3        国内      数据存储       $(get_node_status node-3)"
echo -e " node-4        海外      业务边缘       $(get_node_status node-4)"
echo -e " node-5        海外      数据从库       $(get_node_status node-5)"
echo -e " node-6        全局      异地灾备       $(get_node_status node-6)"
echo -e " node-7        全局      流量调度       $(get_node_status node-7)"
echo -e "${BLUE}============================================================${NC}"
...
local has_global=$(ls ${INVENTORY_DIR}/*.state 2>/dev/null | grep -E "node-4|node-5|node-6" | wc -l)
...
3)
    ...
    echo ""
    echo -e "${CYAN}📝 节点分工与部署清单：${NC}"
    echo -e "  新增节点7（多云业务边缘节点）："
    echo -e "    → 部署：NetBird客户端 + Nginx + NewAPI + 监控探针"
    echo -e "    → 禁止：K3s控制面、数据库类组件"
    echo ""
    echo -e "  新增节点8（多云数据从库节点）："
    echo -e "    → 部署：NetBird客户端 + MySQL从库 + Redis从库 + 备份"
    echo -e "    → 禁止：业务网关、K3s"
    echo ""
    echo -e "${BLUE}============================================================${NC}"
...
4)
    ...
    echo -e "${BLUE}============================================================${NC}"
    echo -e "${YELLOW}📌 服务器选择规则（必须遵守）：${NC}"
    echo -e "  ${RED}❌ 禁止跨区（若加入K3s集群）${NC}：必须与节点1同区域，延迟 < 10ms"
